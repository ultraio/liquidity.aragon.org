...
const UNISWAP_QUERY = `
  query {
    pairs(where: { id: "0x42d52847be255eacee8c3f96b3b223c0b3cc0438" }) {
      id
      totalSupply
      reserveUSD
      token0Price
...
export function useProvideLiquidity() {
  const antContract = useKnownContract('TOKEN_UOS')
  ...
  useEffect(() => {
    ...
    async function fetchTokenInfo() {
      try {
        ...
        if (!cancelled) {
          console.log(results?.data?.pairs[0])
